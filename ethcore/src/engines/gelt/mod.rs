...
/// Gelt BFT consensus engine based off of Tendermint (for now) with round robin proof-of-authority.
...
/// Engine using `Gelt` consensus algorithm, suitable for EVM chain.
pub struct Gelt {
...
impl Gelt {
	/// Create a new instance of Gelt engine
	pub fn new(our_params: GeltParams, machine: EthereumMachine) -> Result<Arc<Self>, Error> {
		let engine = Arc::new(
			Gelt {
...
impl Engine<EthereumMachine> for Gelt {
	fn name(&self) -> &str { "Gelt" }
...
mod tests {
	...
	fn has_valid_metadata() {
		let engine = Spec::new_test_gelt().engine;
	...
	fn seal_submission() {
		...
		let client = generate_dummy_client_with_spec_and_accounts(Spec::new_test_gelt, Some(tap.clone()));
